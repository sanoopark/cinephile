...
export default {
  ...
  computed: {
    ...
    modalStatus() {
      return this.$store.state.searchResults.modalStatus;
    },
    isLoading() {
  ...
  methods: {
    async showMovieDetails(selectedMovieId) {
      await this.$store.commit(
        'searchDetails/saveSelectedMovieId',
        selectedMovieId
      );
      await this.$store.dispatch('searchDetails/fetchSelectedMovieDetails');
      await this.$store.commit('searchResults/changeModalStatus');
    },
    handleIntersect({ isIntersecting }) {
      const isLoading = this.$store.state.searchResults.isLoading === true;
      const isResultsEmpty =
        this.$store.state.searchResults.results.length === 0;

      if (!isIntersecting || isLoading || isResultsEmpty) {
        return;
      }

      this.$store.dispatch('searchResults/fetchNextSearchResults');
    }
  }
};
</script>
